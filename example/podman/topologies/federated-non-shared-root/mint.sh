...
# are free to differ in how deep they go, and here they do:
#
#   the lab department  a root that signs an intermediate, and PVACMS signs with the
#                       intermediate
...
# rotating its root away, on that department's own schedule.
set -euo pipefail
cd "$(dirname "$0")"
...
# its identifier is what every ML certificate will carry.
...
#
# This is the file a site hands to a machine out of band. An IOC stands on its own
# department's segment and can reach nothing beyond it. It is given both roots here, before it
...
# name; the whole one is what establishes trust in it.
{ printf 'LAB_ISSUER=%s\n' "${LAB_ISSUER}"
  printf 'ML_ISSUER=%s\n' "${ML_ISSUER}"
