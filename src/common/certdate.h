 ...
 */
/**
 * The certificate date helper.
 *
 * The time_t <-> ASN1_TIME conversions live in certtime.h; this file adds the
 * CertDate value type and the duration/calendar helpers built on top of them.
 *
 *   certdate.h
 *
 */
...
struct CertDate {
    ...
    CertDate(const std::time_t& time) : t(time), s(timeTToString(time)) {} // NOLINT(*-explicit-constructor)
    // Constructor from ASN1_TIME*
    CertDate(const ASN1_TIME* time) : t(asn1TimeToTimeT(time)), s(timeTToString(t)) {}// NOLINT(*-explicit-constructor), ReSharper disable once CppNonExplicitConvertingConstructor
    // Constructor from ossl_ptr<ASN1_TIME>
    CertDate(const ossl_ptr<ASN1_TIME>& time) : t(asn1TimeToTimeT(time.get())), s(timeTToString(t)) {}// NOLINT(*-explicit-constructor)
    // Constructor from time string
    CertDate(const std::string& time_string) : t(stringToTimeT(time_string)), s(timeTToString(t)) {}// NOLINT(*-explicit-constructor)
     ...
     */
    ossl_ptr<ASN1_TIME> toAsn1_Time() const { return timeTToAsn1Time(t); }
     ...
     */
    static time_t asn1TimeToTimeT(const ASN1_TIME* time) { return certs::asn1TimeToTimeT(time); }
