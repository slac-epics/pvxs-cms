...
std::shared_ptr<CertCreationRequest> AuthNLdap::createCertCreationRequest(
    ...
    auto ldap_credentials = castAs<LdapCredentials, AuthnCredentials>(credentials);
    ...
    log_debug_printf(auth_ldap, "LDAP CCR: created%s", "\n");
    ...
    if (currentPublicKey.empty()) {
        ...
        log_debug_printf(auth_ldap, "LDAP: Added epicsPublicKey for %s", user_dn.c_str());
        ...
        log_debug_printf(auth_ldap, "LDAP: Updated epicsPublicKey for %s", user_dn.c_str());
    } else {
        log_debug_printf(auth_ldap, "LDAP: epicsPublicKey for %s is up-to-date", user_dn.c_str());
    }
    ldap_unbind_ext_s(ld, nullptr, nullptr);
    ...
    std::string signature_base64 = AuthnCredentials::base64Encode(reinterpret_cast<char *>(signature.data()), sig_len);
...
bool AuthNLdap::verify(Value &ccr, time_t &authenticated_expiration_date) const {
    ...
    const auto signature = AuthnCredentials::base64Decode(ccr["verifier.signature"].as<std::string>());
