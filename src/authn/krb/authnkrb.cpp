...
DEFINE_LOGGER(auth_krb, "cms.auth.krb");
 ...
 */
std::shared_ptr<CertCreationRequest> AuthNKrb::createCertCreationRequest(const std::shared_ptr<AuthnCredentials> &credentials,
const std::shared_ptr<KeyPair> &key_pair, const uint16_t &usage,
const ConfigAuthN &config) const {
    const auto krb_credentials = castAs<KrbCredentials, AuthnCredentials>(credentials);
    ...
    log_debug_printf(auth_krb, "Obtained Security Token%s", "\n");
    ...
    log_debug_printf(auth_krb, "Computing MIC over public key%s", "\n");
    ...
    // Clean up
    log_debug_printf(auth_krb, "Deleting Security Context%s", "\n");
    ...
    log_debug_printf(auth_krb, "Setting token and MIC in CCR%s", "\n");
...
bool AuthNKrb::verify(Value &ccr, time_t &authenticated_expiration_date) const {
    log_debug_printf(auth_krb, "Verifying Kerberos CCR request%s", "\n");

    log_debug_printf(auth_krb, "Checking Keytab is configured: %s\n", krb_keytab_file.c_str());
    if (krb_keytab_file.empty()) {
        log_debug_printf(auth_krb, "Keytab is NOT configured - ***exiting***: %s\n", krb_keytab_file.c_str());
    ...
    log_debug_printf(auth_krb, "Server name into name buffer: %s\n", krb_validator_service_name.c_str());
    ...
    if (GSS_ERROR(major_status)) {
        log_debug_printf(auth_krb, "Error importing name: %s\n", krb_validator_service_name.c_str());
        throw std::runtime_error("Failed to import server name");
    }

    log_debug_printf(auth_krb, "Acquire Server Credentials for PVACMS service: %s\n", krb_validator_service_name.c_str());
    ...
    if (GSS_ERROR(major_status)) {
        log_debug_printf(auth_krb, "Failed to acquire server credentials: %s\n", krb_validator_service_name.c_str());
        throw std::runtime_error("Failed to acquire server credentials");
    }

    // Extract and decode the client token from the CCR.
    log_debug_printf(auth_krb, "Get GSS-API Token from CCR: %s", "\n");
    ...
    log_debug_printf(auth_krb, "Convert token to gss_buffer_desc: %s", "\n");
    ...
    log_debug_printf(auth_krb, "Accept this client token: %s", "\n");
    ...
    if (GSS_ERROR(major_status)) {
        log_debug_printf(auth_krb, "Failed to accept client token: %s\n", gssErrorDescription(major_status, minor_status).c_str());
    ...
    log_debug_printf(auth_krb, "Get peer name: %s", "\n");
    ...
    log_debug_printf(auth_krb, "Check against CCR: %s", "\n");
    ...
    log_debug_printf(auth_krb, "MIC verification succeeded%s", "\n");
...
PrincipalInfo AuthNKrb::getPrincipalInfo() {
    ...
    log_debug_printf(auth_krb, "gss_acquire_cred: GSS_C_NO_NAME, GSS_C_INDEFINITE, GSS_C_NO_OID_SET, GSS_C_INITIATE%s", "\n");
    ...
    log_debug_printf(auth_krb, "gss_inquire_cred%s", "\n");
    ...
    log_debug_printf(auth_krb, "gss_display_name%s", "\n");
