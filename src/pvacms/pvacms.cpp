...
;
        ...
        reply["timeStamp.secondsPastEpoch"] = now;
...
Value postCertificateStatus(server::WildcardPV &status_pv,
    ...
    setValue<time_t>(status_value, "renew_by", cert_status.renew_by.t > 0 ? cert_status.renew_by.t : 0);
    setValue<bool>(status_value, "renewal_due", cert_status.renewal_due);
    setValue<time_t>(status_value, "ocsp_status.timeStamp.secondsPastEpoch", now);
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        status_pv.onFirstConnect([&config,
                                  &certs_db,
                                  &cert_auth_pkey,
                                  &cert_auth_cert,
                                  &cert_auth_chain,
        ...
        status_pv.onPut([&config,
            ...
            if (is_revoke && is_own_cert) {
                if (is_admin) {
                    ...
                    op->error(pvxs::SB() << state << " Admin Self-Revoke not permitted on " << our_issuer_id << ":"
                                         << serial << " by " << *creds);
                    return;
                }
            } else if (!is_admin) {
                log_err_printf(pvacms, "PVACMS Client Not Authorised%s", "\n");
                op->error(pvxs::SB() << state << " operation not authorized on " << our_issuer_id << ":" << serial
                                     << " by " << *creds);
                return;
            }
