...
#ifndef PVXS_OCSPSTATUSMONITOR_H
#define PVXS_OCSPSTATUSMONITOR_H
...
class OcspStatusMonitor {
   public:
     ...
     *        verified answer instead of reporting the status as unknown
     */
    OcspStatusMonitor(X509 *trust_anchor, bool hold_last_known);
    ~OcspStatusMonitor();

    OcspStatusMonitor(const OcspStatusMonitor &) = delete;
    OcspStatusMonitor &operator=(const OcspStatusMonitor &) = delete;
    ...
    /**
     * @brief The authority's OCSP status, worked out from the last answer and the moment it runs
     * out.
     *
...
#endif  // PVXS_OCSPSTATUSMONITOR_H
