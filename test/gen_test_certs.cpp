...
struct CertCreator {
    ...
    // Optional inner organizational unit, emitted before the outer one so the subject
    ...
    std::tuple<pvxs::ossl_ptr<EVP_PKEY>, pvxs::ossl_ptr<X509>> create(const bool add_status_extension=true)
    {
        ...
        // This ensures the SKID is computed the same way as OCSP_cert_id_new expects
        ...
        if ( add_status_extension) {
            ...
            addCustomExtensionByNid(cert, cms::ssl::NID_SPvaCertStatusURI, getCertStatusURI(TEST_CERT_PV_PREFIX, issuer_id, serial));
        }
