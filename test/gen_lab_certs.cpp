...
struct CertCreator {
    ...
    std::tuple<pvxs::ossl_ptr<EVP_PKEY>, pvxs::ossl_ptr<X509>> create(const bool add_status_extension=true)
    {
        ...
        // Use keyid:always to copy the issuer's SKID into AKID
        ...
        if ( add_status_extension) {
            ...
            addCustomExtensionByNid(cert, cms::ssl::NID_SPvaCertStatusURI, getCertStatusURI(cert_pv_prefix, issuer_id, serial));
        }
...
void usage(const char* argv0) {
    std::cerr<<"Usage: "<<argv0<<" [-O <outdir>] [-R <cn>] [-L <cn>] [-M <cn>] [-S <uri>]\n"
               ...
               "      * a Controls (Lab) intermediate  (status extension, distinct CN)\n"
               "      * an ML intermediate             (status extension, distinct CN)\n"
               "\n"
               "    Writes, into <outdir>:\n"
...
int main(int argc, char *argv[])
{
    try {
        ...
        outdir.push_back('/');
        if(optind!=argc) { usage(argv[0]); std::cerr<<"\nUnexpected arguments\n"; return 1; }

        serial_number_t serial = TEST_FIRST_SERIAL;

        // 1. Facility Root Certificate Authority - self-signed, NO status extension.
        //    (A CA loaded by PVACMS with no status extension is accepted silently, so the
        //     root has no CERT:STATUS PV, exactly as required.)
        ...
        if (!root_ocsp_uri.empty()) {
            ...
            // The responder reads the authority's certificate status from this file. It is written here
        ...
        auto mintIntermediate = [&](const std::string& cn, const char* p12name) -> std::string {
            ...
            // last chain element. A single-element chain of [root] satisfies this.
            MUST(1, sk_X509_push(p12.cacerts.get(), root_cert.get()));
            p12.write(p12name);
            ...
            return cms::cert::CertStatus::getFullSkId(cert.get());
        };
        ...
        const std::string lab_short = lab_issuer.substr(0, cms::cert::kIssuerIdNameLength);
        const std::string ml_short  = ml_issuer.substr(0, cms::cert::kIssuerIdNameLength);

        if (lab_issuer == ml_issuer) {
            throw std::runtime_error("Lab and ML intermediates produced the same issuer id - "
                                     "each MUST have a distinct key/SKID");
        }

        // Emit the two issuer ids: to a file the ca-keygen job packages into a ConfigMap, and
        // to stdout so the job log shows them.
        {
